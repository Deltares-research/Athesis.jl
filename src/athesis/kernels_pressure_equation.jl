...
########## CPU kernels
@inline function pressure_kernel(i, j, source::Array, h::Array, u::Array, v::Array, hⁿ⁺¹::Array, uⁿ⁺¹::Array, vⁿ⁺¹::Array, Δx::Float64, Δy::Float64, Δt::Float64, K::Array)
    # 2D implementation (2DH or 2DV)
    # for now cell centered
    F = -K[i,j] * ( (h[i+1,j] + h[i-1,j] - 2.0*h[i,j])/(Δx*Δx) + (h[i,j+1]+h[i,j-1] - 2.0*h[i,j])/(Δy*Δy) ) + source[i,j]
...
@inline function pressure_kernel(i, j, k, source::Array{Float64,3}, h::Array{Float64,3}, u::Array{Float64,3}, v::Array{Float64,3}, w::Array{Float64,3}, hⁿ⁺¹::Array{Float64,3}, uⁿ⁺¹::Array{Float64,3}, vⁿ⁺¹::Array{Float64,3}, wⁿ⁺¹::Array{Float64,3}, Δx::Float64, Δy::Float64, Δz::Float64, Δt::Float64, K::Array{Float64,3})
    # 3D implementation
    # for now cell centered
    F = -K[i,j,k] * (
